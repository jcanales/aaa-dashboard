...
export function AbiCrossingsPage() {
  ...
  const load = (p: number) => {
    setLoading(true)
    fetchCrossings({ coKey: selectedClient?.coKey, dateFrom, dateTo, page: p, limit: 25 })
          ...
          <table className="w-full text-xs sticky-table">
            <thead>
              <tr className="text-slate-500 border-b border-slate-100">
                <th className="text-left px-4 py-2.5 font-medium">Traffic #</th>
                ...
                <th className="text-left px-4 py-2.5 font-medium">Pedimento</th>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={8} className="text-center py-8 text-slate-400">
                    {loading ? 'Loading…' : 'No crossings found'}
                  </td>
                </tr>
              )}
              {rows.map((r) => (
                <tr key={r.recid} className="border-b border-slate-50 hover:bg-slate-50">
                  <td className="px-4 py-2.5 font-medium text-slate-800">{r.trafficNo}</td>
                  ...
                  <td className="px-4 py-2.5 text-slate-500">{r.pedimento || '—'}</td>
                  <td className="px-4 py-2.5 text-right text-slate-700">{r.value ? fmtUSD(r.value) : '—'}</td>
